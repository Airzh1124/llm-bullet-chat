...
LLM Bullet Chat 是一个 Windows 桌面弹幕 overlay 原型。它会本地截取屏幕、用 OCR 提取文字上下文、先做本地隐私脱敏，然后调用 DeepSeek 生成类似直播间观众的弹幕，并通过 PyQt6 透明置顶窗口显示在屏幕上。

这个项目的目标是做一个轻量的 AI 弹幕层，而不是录屏或视觉上传工具。默认情况下，截图只留在本地；只有经过本地隐私脱敏后的 OCR 文本会发送给 LLM。
...
- 可配置字体、颜色、描边、速度、出现区域、透明度和最大数量。
- 可在 OCR 前遮掉弹幕区域，避免生成的弹幕被读回上下文。
- CSV audit log 记录采集、脱敏、上传和生成链路。
...
首次安装 RapidOCR / onnxruntime 可能会稍慢。
...
源码里的 LLM prompt 使用英文编写，中文示例在 Python 代码里用 Unicode escape 保存，用来减少 Windows 终端和编辑器编码差异带来的乱码问题。
...
- `OVERLAY_KEEP_TOP_INTERVAL_MS`: 周期性重新置顶；设为 `0` 可关闭。
