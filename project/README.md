...
- `DANMAKU_OUTLINE_COLOR`：文字描边颜色，默认黑色。
- `DANMAKU_OUTLINE_WIDTH`：文字描边宽度。复杂背景看不清时调到 `5` 或 `6`。
- `DANMAKU_TEXT_BACKGROUND_ALPHA`：`floating` 模式下每条弹幕后面的半透明黑底，`0` 关闭，`60` 到 `100` 会更稳。
...
- `DANMAKU_EXCLUDE_FROM_OCR`：默认开启。OCR 和画面变化检测前会遮掉弹幕区域，避免弹幕被读回去形成反馈循环。
